...
| Использовать ллм для логичных замен | модель находит, детерминированный механизм заменяет: сканер определяет колонки с ПДн и через код настраивает myanon, детектор ищет имена в свободном тексте. Что из этого уже работает, а что нет — в разделе «Состояние работ», без округлений | сканер воспроизвёл ручной конфиг: 3 колонки, 14 отклонено фильтрами. Детектор: 10 из 10 найдено, 0 ложных на 13 отрицательных примерах |
...
Это ровно те три колонки, которые до сих пор были прописаны в конфиге руками.
**Автонастройка воспроизвела ручной конфиг** — это и есть проверка, что она
работает, а не просто выдаёт правдоподобный список.
...
| Сканер колонок (шаг 1 цикла) | **работает**: на демо-базе воспроизвёл ручной конфиг |
