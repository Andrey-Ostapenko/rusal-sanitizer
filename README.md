...
```
ФАЗА НАСТРОЙКИ — один раз на новую базу

   исходный дамп ──► сканер: модель называет колонки с ПДн
                        │
                        ▼
                     сборка конфига детерминированным кодом
                        │
                        ▼
                     прогон + проверка: что осталось?
                        │
              ┌─────────┴──────────┐
        нашлось ещё            чисто
              │                    │
              └──► новый виток     ▼
                              человек принимает дифф → конфиг


ФАЗА ПРОГОНА — по готовому конфигу, модель не нужна
   ...
   база MySQL ──mysqldump──► исходный дамп ─┐
                   ...
                   (колонки, перечисленные  │
                    в конфиге)              │
                                  │         │
                                  ▼         │
                   Этап 2: слой подстановки ◄┘
                   весь дамп как текст:
                     • разносит замены этапа 1 по тексту
                     • телефон, ИНН, СНИЛС, почта по формату
...
Граф снят с кода разбором импортов, а не нарисован по памяти.

```mermaid
graph TD
    subgraph Основания["Основания — своих зависимостей нет"]
    ...
    subgraph Настройка["Фаза настройки"]
        scan["llm_scan_columns<br/>модель называет колонки с ПДн"]
    end
    ...
    subgraph Проверка["Проверка"]
        verify["verify<br/>семь проверок результата"]
        determ["check_determinism<br/>тот же ключ — тот же результат"]
        control["control_check<br/>проверка самого эталона"]
        detect["llm_detect<br/>детектор имён + замер"]
    end

    gen["generate_demo_dump<br/>демо-данные"]
...
Оркестрация — не в Python: `sanitize.sh` запускает myanon и `text_substitute`,
`entrypoint.sh` замыкает цепочку от базы до базы и зовёт проверки,
`docker-compose.yml` поднимает всё вместе.

**`pii_patterns` — узел, от которого зависят пятеро.** Это намеренно: правило
...
**Оговорка, которую нашёл этот же разбор.** `verify` импортирует из
