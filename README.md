...
| Сканер колонок | нет, хотя промпт русский | на sakila принято 8 колонок, все с английскими именами |
...
Из-за этой правки счёт «принято 8, отклонено 44» смещается на одну колонку:
`staff.picture` теперь отбрасывается фильтром по типу, а не рассматривается
моделью. Числа выше — замер до правки; пересчитывать их значит заново
загружать sakila и звать модель, и до пересдачи это не сделано.
