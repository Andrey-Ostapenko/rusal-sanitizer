...
Она поднимает MySQL, готовит исходную базу, выгружает, санитизирует, загружает
результат во вторую базу и печатает восемь проверок. Подробности и включение
...
Если этот репозиторий читает не человек, а модель, точка входа для неё —
[`llms.txt`](llms.txt) в корне: та же карта, но плотнее и со ссылками на файлы.
Это не замена README, а дополнение: по соглашению `llms.txt` README не
заменяет, а разгружает.
...
нечитаемой. Так же не показан `pii_columns`: его импортируют трое —
`scan_columns`, `text_substitute` и `verify`, и это ровно та развязка
проверяющего с проверяемым, о которой ниже. Он появился при переходе на
...
**`pii_patterns` — узел, от которого зависят пятеро** (`control_check`, `detect`, `scan_columns`, `text_substitute`, `verify`). Это намеренно: правило
...
|---|---|---|---|
| ФИО | myanon по конфигу | `texthash 30` | непустота и уникальность; длина НЕ сохраняется — было 11–17, стало ровно 30 |
| Адрес | myanon по конфигу | `texthash 30` | непустота и уникальность; длина НЕ сохраняется — было 41–47, стало ровно 30 |
| Телефон | слой подстановки, по формату | порождённый номер | префикс `+79` и длина в цифрах; разделители теряются: `+7 912 345-67-89` → `+79436133515` |
...
| Имя в свободном тексте | слой подстановки, по находке модели | буквенная строка | длина в пределах 5–20 знаков: `fake_name` зажимает её, и «Гуськовой Ниной Петровной» (24 знака) станет 20 |
...
как именно схлопывается: одиннадцать разных клиентов получили «Смирнова Елена»,
...
Восемь проверок. Вывод последнего сквозного прогона:
