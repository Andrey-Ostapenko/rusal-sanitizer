...
дамп потоком, к рабочей базе не подключается, заменяет значения в перечисленных
колонках. Одинаковое значение всегда получает одинаковую замену — это встроено
в его механику, а не настраивается вручную.
