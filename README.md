...
| Проресерчить опенсорс и доработать | myanon выбран из пяти кандидатов и доработан вторым слоем | после одного лишь myanon в дампе оставалось 131 исходное значение из 186 |
| Использовать ллм для логичных замен | сегодня в конвейере: модель порождает пул правдоподобных ФИО, детерминированный хеш его раздаёт. Написан и замерен детектор имён в свободном тексте, в конвейер пока не встроен — см. «Состояние работ» | детектор: 10 из 10 найдено, 0 ложных на 13 отрицательных примерах |
...
Вход и выход — база MySQL. Между ними два этапа поверх дампа.

```
   база MySQL ──mysqldump──► исходный дамп
                                  │
            ┌─────────────────────┤
            │                     ▼
            │   Этап 1: myanon по конфигу
            │   (почта и адрес в колонках)
            │                     │
            │                     ▼
            └────────────► Этап 2: доработка
                (весь дамп как текст: разносит замены
                 этапа 1 + телефон, ИНН, СНИЛС по формату
                 + имена в свободном тексте)
                                  │
                                  ▼
                 очищенный дамп ──mysql──► очищенная база
```

Вся цепочка от базы до базы запускается одной командой и не требует ручных
шагов.

### Этап 1 — открытый инструмент

**myanon** (лицензия Modified BSD): читает дамп потоком, к рабочей базе не
подключается, заменяет значения в перечисленных колонках. Одинаковое значение
всегда получает одинаковую замену — это встроено в его механику, а не
настраивается вручную.

Выбран из пяти кандидатов. Отсеяны по проверяемым признакам: gdpr-dump —
лицензия GPL-3.0 обязывает открывать производный код; Greenmask — поддержка
MySQL в пререлизном состоянии; nxs-data-anonymizer — разработка остановлена;
Neosync — архивирован; Datanymizer — MySQL в планах.
...
| Вид данных | Чем заменяется | Что сохраняется |
|---|---|---|
| Почта | `emailhash` | домен, форма адреса |
| Адрес | `texthash` | длина, признак заполненности |
| Телефон | порождённый номер | формат `+7 9XX XXX-XX-XX` |
| ИНН (12 знаков) | порождённый ИНН | обе контрольные цифры верны |
| СНИЛС | порождённый СНИЛС | контрольное число верно |
| ФИО | см. «Состояние работ» ниже | — |

Телефон, ИНН и СНИЛС обрабатывает второй этап, а не конфиг: он работает с
текстом всего дампа, поэтому заменяет их одинаково и в своей колонке, и там,
где они упомянуты строкой. Два разных механизма для одного вида данных дали бы
два разных результата в одной базе — так и было в ранней версии, где телефон в
колонке превращался в буквенный мусор, а в тексте — в правильный номер.
...
### Две разные роли модели — и требования к запуску у них противоположные

**Роль 1 — настройка.** Схема базы между прогонами не меняется, поэтому
перечень обрабатываемых колонок определяется один раз, человек его принимает,
дальше он лежит файлом. Модель нужна только при наведении решения на новую базу.

**Роль 2 — поиск имён в свободном тексте.** Комментарии в каждой выгрузке
новые, кэшировать нечего, поэтому модель вызывается на каждом прогоне и в
целевом состоянии становится требованием к промышленной установке. Порядка
10 ГБ оперативной памяти на машине с моделью; эндпоинт настраиваемый, поэтому
модель может стоять на отдельной машине внутри контура.
...
| Компонент | Состояние |
|---|---|
| Сквозной прогон база → база в контейнере | работает, проверено машинно |
...
| Детектор имён | написан и замерен, в конвейер не встроен |
| Замена ФИО | в переходном состоянии, см. ниже |
| Цикл автонастройки конфига | спроектирован, не реализован |
...
Исходный код решения, демо-данные и пара дампов «до и после» готовы и
публикуются по запросу — включая ключ, которым получена пара, чтобы результат
можно было воспроизвести побайтово, а не принять на веру.

---

## Использованное открытое программное обеспечение

**myanon** — анонимизатор дампов MySQL.

- Источник: https://github.com/ppomes/myanon
- Автор: Pierre Pomes
- Лицензия: Modified BSD
- Использован коммит `5eaafb182cbdc58c36677dff512ee956317677c1`
