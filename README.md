...
4. **Очищенную базу читают люди или она вход для машинной обработки?**
   ...
   2–3 склейки (замерено: 1). Их ловит проверка разнообразия, но ловит
...
Это вывод контейнера как есть: снят только префикс, которым `docker compose`
помечает строки, ничего не выброшено и не переставлено. Сама расшифровка тоже
...
проверка детерминированности повторяет санитизацию ещё дважды, и каждый раз
модель вызывается заново на всех комментариях.
