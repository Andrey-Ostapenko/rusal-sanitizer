...
Отдельно сказано: критерий по разнообразию данных будет зачтён частично.
Это принято и учтено — см. «Границы решения».
...
обрабатывает myanon той же механикой, что и адрес. После правки — семь
...
| Замыкание цикла (шаги 3–4) | один виток пройден целиком: сканер → сборщик → прогон → семь проверок. Автоматического повторения витков нет, повторный запуск делается руками |
...
--build`, код возврата 0, семь проверок «норма» с ненулевыми числами,
