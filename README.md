...
Ответы нанимающего от 15.09: срок перенесён на 24 сентября; базу заказчик не
присылает — публичная или своя, обязательно с PII-полями и свободным текстом;
перечня конфиденциальных данных нет, трактовка на исполнителе, чистятся обе
категории; MySQL фиксирован, переносимость не нужна; сдача — репозиторий,
...
| Опенсорс и доработка | myanon выбран из пяти кандидатов, доработан вторым слоем | после одного лишь myanon в дампе оставалось 131 значение из 186 |
| Роль ЛЛМ | модель находит, детерминированный механизм заменяет | сканер: 3 колонки принято, 14 отклонено. Детектор: 10 из 10, 0 ложных |
...
`pii_patterns` показан на обеих схемах — это один и тот же модуль. `detect`
вызывается слоем подстановки только при `DETECT_NAMES=1`.
...
| `pii_columns` | перечень персональных колонок: общие данные для слоя замены и для проверки |
...
изменился, семь проверок по-прежнему «норма».
...
обрабатывает myanon той же механикой, что и адрес. После правки все семь
проверок «норма», расхождений 0.
