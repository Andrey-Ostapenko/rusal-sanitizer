...
контрольными суммами. Сущность `PERSON` есть, но опирается на подключённую
NER-модель, то есть возвращает вопрос о качестве на русском туда же, откуда
он взялся. Итог: Presidio закрыл бы лёгкую половину задачи и не закрыл бы
трудную — ИНН и СНИЛС пришлось бы писать самим, как и написаны, а имена всё
равно остались бы за моделью.
