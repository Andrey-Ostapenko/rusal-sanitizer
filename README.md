...
падали, а пять печатали «норма», не сравнив ни одного значения. Исправлено:
у проверки теперь три исхода, а не два.
...
--skip-extended-insert`. Отдельно: в sakila есть `staff.picture` типа
`mediumblob`,
...
`pii_columns._самопроверка`.

После правки sakila загружена и просканирована заново: `staff.picture`
действительно отбрасывается фильтром по типу — строка
«тип mediumblob — двоичные данные» есть в отчёте.

Честно о разнице в числах: прежний замер давал 8 принятых, сегодняшний даёт 9.
Добавилась `staff.username`. Правка перечня типов этого объяснить не может —
она касается только `staff.picture`. Между прогонами отличались ещё две вещи:
sakila бралась из другого источника и модель отвечала в другой день. Какая из
них дала разницу, не установлено, и выдавать догадку за причину здесь нечего.
Сам факт полезен: он показывает, что фаза настройки не детерминирована в той
же мере, что фаза прогона, — поэтому конфиг и принимает человек по диффу.
