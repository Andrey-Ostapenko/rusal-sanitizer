...
Проверяется:
  ...
  6. Одно значение не получило двух разных замен.
  7. Внешние ключи указывают на существующие строки родительских таблиц.
  8. Порождённые ИНН и СНИЛС проходят проверку контрольной суммы.

У каждой проверки три исхода: норма, НАРУШЕНО и НЕ ПРОВЕРЕНО — последний
означает, что сравнивать было нечего, и тоже даёт ненулевой код возврата.

Использование:
    python3 -m sanitizer.checks.verify исходный.sql результат.sql
"""
import re
import sys
