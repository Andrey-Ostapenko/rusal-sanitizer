...
def test_null_не_показывается_модели_как_образец():
    """
    Замерено на sakila: единственный `NULL` среди образцов соседней колонки
    менял ответ модели по `staff.username` — промпт строится на всю таблицу.
    """
    строки = [["'Mike'"], ["NULL"], ["'Jon'"]]
    assert scan_columns.образцы(строки, 0) == ["Mike", "Jon"]
    assert scan_columns.образцы([["NULL"], ["null"]], 0) == []


def test_образцы_без_повторов():
    assert scan_columns.образцы([["'x'"], ["'x'"], ["'y'"]], 0) == ["x", "y"]
